##Regenie-2  Pairs Matching	(Number of incorrect matches in round.2.1 -> R_Test1_PairsMatching_f399_2)
...
odir="/data/home/hmy431/outputs/PairsMatching_2"     #Output directory
...
./regenie_v2.2.4.gz_x86_64_Centos7_mkl  \
  ...
  --phenoFile ${ddir}/PairsMatching_2.txt \
  --phenoColList PairsMatching_2_logrz \
  ...
  --pred ${odir}/ukb_step1_Pairs_2_pred.list \
  --bsize 1000 \
  --threads 8 \
  --strict \
  --minMAC 10 \
...
cp ${odir}/ukb_step2_chr1_PairsMatching_2_logrz.regenie ${odir}/ukb_step2_Pairs_2.regenie
for i in {2..22}; do
cp ${odir}/ukb_step2_chr${i}_PairsMatching_2_logrz.regenie ${odir}/ukb_step2_chr${i}
sed -i '1d' ${odir}/ukb_step2_chr${i}
cat ${odir}/ukb_step2_chr${i} >> ${odir}/ukb_step2_Pairs_2.regenie
