...
odir="/data/home/hmy431/outputs/DemVas"         	          #Output directory
...
./regenie_v2.2.4.gz_x86_64_Centos7_mkl  \
  ...
  --firth 0.01 --approx \
  --firth-se --approx \
  --pred ${odir}/ukb_step1_DemVas_pred.list \
  --bsize 1000 \
  --threads 8 \
  --strict \
  --minMAC 10 \
...
cp ${odir}/ukb_step2_chr1_DemVas.regenie ${odir}/ukb_step2_DemVas.regenie
for i in {2..22}; do
cp ${odir}/ukb_step2_chr${i}_DemVas.regenie ${odir}/ukb_step2_chr${i}
sed -i '1d' ${odir}/ukb_step2_chr${i}
cat ${odir}/ukb_step2_chr${i} >> ${odir}/ukb_step2_DemVas.regenie
