##Preparing Regenie output  *Symbol digit substitution	(Number of symbol digit matches made correctly.2.0) 
...
#Adding MAF
DB1 <- read.table("SymbolDigit_2/ukb_step2_SDS_2.regenie", header=TRUE)
...
#Save
saveRDS(DB, file='SymbolDigit_2/ukb_step2_SDS_2_add.rds')
#Tab delimited text file
write.table(DB, file = "SymbolDigit_2/ukb_step2_SDS_2_add.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
#Compressed file
write.table(DB, gzfile("SymbolDigit_2/ukb_step2_SDS_2_add.txt.gz"), row.names = FALSE, col.names = TRUE, quote = FALSE) 