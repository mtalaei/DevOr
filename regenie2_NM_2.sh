##Regenie-2  Numeric memory (Maximum digits remembered correctly.2.0)
...
odir="/data/home/hmy431/outputs/NumericMemory_2"         #Output directory
...
./regenie_v2.2.4.gz_x86_64_Centos7_mkl  \
  ...
  --phenoFile ${ddir}/NumericMemory_2.txt \
  --phenoColList NumericMemory_2_z \
  ...
  --pred ${odir}/ukb_step1_NM_2_pred.list \
  --bsize 1000 \
  --threads 8 \
  --strict \
  --minMAC 10 \
...
cp ${odir}/ukb_step2_chr1_NumericMemory_2_z.regenie ${odir}/ukb_step2_NM_2.regenie
for i in {2..22}; do
cp ${odir}/ukb_step2_chr${i}_NumericMemory_2_z.regenie ${odir}/ukb_step2_chr${i}
sed -i '1d' ${odir}/ukb_step2_chr${i}
cat ${odir}/ukb_step2_chr${i} >> ${odir}/ukb_step2_NM_2.regenie
